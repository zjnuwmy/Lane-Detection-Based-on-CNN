...
IMAGE_HEIGHT, IMAGE_WIDTH = 40, 70
...
class Preprocess(object):
    ...
    #这里必须要输进去图片，而不是数组，这点要格外注意！！！（即这里不能用shape）
    def crop(self, sample):
        image, target = sample['image'], sample['target']     
        #w, h = image.size
        #w, h = cv2.GetSize(sample)
        w, h ,_ = image.shape
        #h = image.size[1]  
...
class ToTensor(object):
    ...
    def __call__(self, image):
        ...
        print("before_utils_image:",np.array(image))
        print("before_utils_shape_image:",np.array(image).shape)
        #print("before_utils_labels:",np.array(sample["target"]))
        #target = torch.tensor(float(target))
        
        image = tf.to_tensor(image)
        print("last_utils_image:",np.array(image))
        print("last_utils_shape_image:",np.array(image).shape)
        #print("last_utils_labels:",np.array(sample["target"]))
